...
export const Hero: React.FC = () => {
  ...
  // Mobile stats — 3-col grid (key lead-gen numbers)
  const mobileStats = [
    { value: '15,000+', label: 'Students' },
    { value: '95%', label: 'Visa Success' },
    { value: '500+', label: 'Universities' },
  ...
  return (
    <section className="bg-white w-full" style={{ paddingTop: '80px' }}>

      {/* ══════════════════════════════════════════════
          MOBILE HERO — only visible on < lg
          Lead generation first, image below
      ══════════════════════════════════════════════ */}
      <div className="lg:hidden px-4 pt-6 pb-8">
        ...
        >
          ...
          <motion.div variants={itemVariants} className="inline-flex items-center self-start mb-4">
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[#6D4AFF]/8 border border-[#6D4AFF]/15">
              ...
              <span className="text-[10px] font-semibold text-[#6D4AFF] tracking-wide">
          ...
          <motion.h1
            ...
            className="font-extrabold text-[#0F172A] leading-[1.06] mb-3"
          ...
          <motion.p
            variants={itemVariants}
            className="text-[14px] leading-[1.65] text-[#64748B] mb-6"
          >
            Find universities, scholarships, country matches and visa guidance in one platform.
          </motion.p>
          ...
          <motion.a
            variants={itemVariants}
            href="/assessment"
            className="flex items-center justify-center gap-2 w-full py-4 rounded-xl text-[14px] font-bold text-white bg-[#6D4AFF] hover:bg-[#5B3BE0] transition-all duration-200 shadow-lg shadow-[#6D4AFF]/25 mb-3"
          >
          ...
          <motion.a
            ...
            className="flex items-center justify-center gap-2 w-full py-3.5 rounded-xl text-[14px] font-semibold text-[#0F172A] bg-white border border-slate-200 hover:border-slate-300 hover:bg-slate-50 transition-all duration-200 mb-6"
          >
            <Calendar className="w-4 h-4" />
            Book Free Consultation
          </motion.a>

          {/* Section 6: Social Proof — 3-column grid */}
          <motion.div
            variants={itemVariants}
            className="grid grid-cols-3 gap-2 py-5 border-y border-slate-100 mb-6"
          >
            {mobileStats.map((s) => (
              <div key={s.label} className="text-center">
                <div className="text-[20px] font-extrabold text-[#0F172A] leading-none">{s.value}</div>
                <div className="text-[9px] font-medium text-[#64748B] mt-1 leading-tight">{s.label}</div>
              </div>
            ))}
          </motion.div>

          {/* Section 7: Hero Image — below CTAs, lazy loaded */}
          <motion.div
            variants={itemVariants}
      ...
      {/* ══════════════════════════════════════════════
          DESKTOP HERO — only visible on >= lg
          Unchanged from original
      ══════════════════════════════════════════════ */}
      <div className="hidden lg:block overflow-hidden">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-10">
          <div
            className="grid lg:grid-cols-[1fr_420px_220px] lg:items-stretch"
